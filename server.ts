...
import { isSafeExternalUrl } from './src/utils/security.ts';
...
async function dispatchWebhooks(event: 'certificate.created' | 'certificate.approved' | 'certificate.cancelled' | 'citizen.registered', data: any) {
  ...
  for (const hook of targets) {
    if (!isSafeExternalUrl(hook.url)) continue;
    try {
...
`;
  ...
  app.post("/api/admin/apps-script-sync", async (req, res) => {
    try {
      ...
      if (!targetUrl || !isSafeExternalUrl(targetUrl)) {
        return res.status(400).json({
          success: false,
          message: "Google Apps Script WebApp URL সঠিক বা নিরাপদ নয়। অনুগ্রহ করে একটি বৈধ ও নিরাপদ WebApp URL প্রদান করুন।"
        });
      }
  ...
  app.post("/api/admin/webhooks", (req, res) => {
    ...
    if (!url || !isSafeExternalUrl(url)) {
      return res.status(400).json({ success: false, message: "একটি বৈধ ও নিরাপদ Webhook URL প্রদান করুন (http:// বা https://)।" });
    }
  ...
  app.post("/api/admin/webhooks/test", async (req, res) => {
    ...
    if (!targetUrl || !isSafeExternalUrl(targetUrl)) {
      return res.status(400).json({ success: false, message: "নিরাপত্তাজনিত কারণে private/internal Webhook URL বা অকার্যকর URL অনুমোদিত নয়।" });
    }
