...
export const CitizenMasterRegister: React.FC<CitizenMasterRegisterProps> = ({
  ...
  // Memoize summary stats in a single O(N) pass to prevent 3 unmemoized array allocations & full iterations on every keystroke
  const stats = useMemo(() => {
    let maleCount = 0;
    let femaleCount = 0;
  ...
  // Real-time filtering logic: searchQuery normalization is hoisted outside the filter loop for O(1) string creation per search update
